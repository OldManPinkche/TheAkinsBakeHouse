...
async function findActiveLocationId(accessToken) {
...
async function createSquarePaymentLink(accessToken, squarePayload) {
...
exports.handler = async (event) => {
  ...
  const configuredLocationId = cleanText(process.env.SQUARE_LOCATION_ID);

  if (!accessToken) {
    return json(500, {
      message: "Square is not configured yet. Add SQUARE_ACCESS_TOKEN in the hosting environment."
    });
  }
  ...
  const squarePayload = {
    ...
    order: {
      location_id: configuredLocationId || "auto",
      source: {
  ...
  if (!configuredLocationId || configuredLocationId.toLowerCase() === "auto") {
    const locationResult = await findActiveLocationId(accessToken);
  ...
  let { response: squareResponse, data: squareData } = await createSquarePaymentLink(accessToken, squarePayload);
  ...
  if (!squareResponse.ok && hasInvalidLocationError(squareData)) {
    ...
    if (locationResult.locationId) {
      ...
      squarePayload.order.location_id = locationResult.locationId;
      ({ response: squareResponse, data: squareData } = await createSquarePaymentLink(accessToken, squarePayload));
    }
  }
